...
def scale_down(workloads, namespace):
    for deployment in workloads:
        try:
            ...
            # scaling down
            logger.info('new replica is ', {calculate_scaling_down})
            if calculate_scaling_down < 2:
                logger.info("is too low for scaling down", {calculate_scaling_down})
                break
...
logger.info("scaled down time in IST:", ist_now.strftime('%Y-%m-%d %H:%M:%S %Z%z'))
slack_alert(f"{user_input_list} has been scaled down.")
